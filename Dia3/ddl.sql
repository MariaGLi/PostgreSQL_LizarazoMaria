...
alter table usuarios add constraint fk_username foreign key(username)
references credenciales(username);
...
select * from credenciales where sha512('2lz&3%m3fm9lc'::bytea) = password_sha and username = 'talk39';

-- NO retorna nada debido a que la contraseña es errónea para el username
select * from credenciales where sha512('2lz&3%bhw234%'::bytea) = password_sha and username = 'talk39';

update credenciales set password = sha512(generate_random_password2(12)) where username = 'brush99';
update credenciales set password = generate_random_password2(12);

select string_agg(SUBSTRING('0123456789abcdefghijklmnñopqrstuvwxyz@$#%&?¡!¿', round(random() * length('0123456789abcdefghijklmnñopqrstuvwxyz@$#%&?¡!¿'))::integer, 1), '') 
as newPassword from generate_series(0, 5);
...
BEGIN
	...
	as newPassword from generate_series(0, long); -- queda para revisar
END;
$$ language plpgsql;
...
alter table usuarios rename column pass to username;
...
-- Ejemplo por si no se puede hacer directamente en la tabla o se olvida ponerlo.
alter table integrante add constraint check_edad check(edad > 9);

insert into integrante(nombre, edad) values ('rafael', 11);
insert into integrante(nombre, edad) values ('pedro', 8);

select * from integrante;

INSERT INTO usuarios (nombre, apellido, correo, last_location, ciudad, link_blog, username, seguidores, siguiendo)
VALUES
...
select row_number() over(order by id_user), id_user, username from usuarios;
select nombre, id_user, pass from usuarios where ciudad = 'Colombia';